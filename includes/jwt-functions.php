...
function trusona_base64url_encode($data)
{
...
function trusona_base64url_decode($data, $strict = false)
{
  $b64 = strtr($data, '-_', '+/');
  return base64_decode($b64, $strict);
}

function trusona_extract_algorithm($header) {
  if(!isset($header->alg)) {
    return NULL;
  }
...
function trusona_not_expired($payload) {
...
function trusona_matches_issuer($payload) {
...
function trusona_is_valid_jwt($token, $secret)
{
  try {
    ...
    $header = json_decode(trusona_base64url_decode($first));
    if(!$header) {
      return false;
    }
    
    $algorithm = trusona_extract_algorithm($header);
    if(!$algorithm) {
      return false;
    }
    
    $payload = json_decode(trusona_base64url_decode($second));
    if(!$payload) {
      return false;
    }
    
    $signature = trusona_base64url_encode(hash_hmac($algorithm, "$first.$second", $secret, true));
